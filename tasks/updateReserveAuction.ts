...
// yarn hardhat --network local updateReserveAuction --sbtid 2 --auctionid 3 --reserveprice 300

task("updateReserveAuction", "updateReserveAuction a dNFT to market place function")
.addParam("sbtid", "soul bound token id ")
.addParam("auctionid", "auction id")
.addParam("reserveprice", "new reserve price ")
.setAction(async ({sbtid, auctionid, reserveprice}: {sbtid:number, auctionid:number, reserveprice: number}, hre) =>  {
  runtimeHRE = hre;
  ...
  console.log(
    ...
    const receipt = await waitForTx(
        market.connect(seller).updateReserveAuction(
            auctionid,
            reserveprice
    ));

    let eventsLib = await new Events__factory(deployer).deploy();
    const event = findEvent(receipt, 'ReserveAuctionUpdated', eventsLib);
    console.log(
      "\n\t--- updateReserveAuction success! Event ReserveAuctionUpdated emited ..."
    );
