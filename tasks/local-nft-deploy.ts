...
import { BigNumber } from 'ethers';
  ...
  // yarn local-nft-deploy

  task('local-nft-deploy', 'deploys the BAYC NFT to local testnet').setAction(async ({}, hre) => {
        ...
        console.log(" let user2= '", user2.address, "'");
        console.log(" let user3= ", user3.address, "'");
...
*/
        ...
        await waitForTx(await apeNFT.connect(deployer).mint(user.address, tokenId));
        await waitForTx(await apeNFT.connect(deployer).setTokenURI(tokenId, uri));
  
        tokenId =2;
        uri = baseURI + tokenId + ".png";
        await waitForTx(await apeNFT.mint(user2.address, tokenId));
        await waitForTx(await apeNFT.setTokenURI(tokenId, uri));

        tokenId =3;
        uri = baseURI + tokenId + ".png";
        await waitForTx(await apeNFT.mint(user3.address, tokenId));
        await waitForTx(await apeNFT.setTokenURI(tokenId, uri));

        tokenId=4;
        uri = baseURI + tokenId + ".png";
        await waitForTx(await apeNFT.mint(user4.address, tokenId));
        await waitForTx(await apeNFT.setTokenURI(tokenId, uri));

        tokenId=5;
        uri = baseURI + tokenId + ".png";
        await waitForTx(await apeNFT.mint(user5.address, tokenId));
        await waitForTx(await apeNFT.setTokenURI(tokenId, uri));

        tokenId=6;
        uri = baseURI + tokenId + ".png";
        await waitForTx(await apeNFT.mint(user6.address, tokenId));
        await waitForTx(await apeNFT.setTokenURI(tokenId, uri));

        tokenId=7;
        uri = baseURI + tokenId + ".png";
        await waitForTx(await apeNFT.mint(user7.address, tokenId));
        await waitForTx(await apeNFT.setTokenURI(tokenId, uri));
        

        let tokenURI  = await apeNFT.tokenURI(5);
        ...
        let owner = await apeNFT.ownerOf(5);
        console.log(`\t-- TokenID: 5 , NFT owner is `,  owner);
