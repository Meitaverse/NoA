...
export class DerivativeNFTImageURI extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    ...
    assert(
      id != null,
      "Cannot save DerivativeNFTImageURI entity without an ID"
    );
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type DerivativeNFTImageURI must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("DerivativeNFTImageURI", id.toString(), this);
    }
  }

  static load(id: string): DerivativeNFTImageURI | null {
    return changetype<DerivativeNFTImageURI | null>(
      store.get("DerivativeNFTImageURI", id)
    );
  }
