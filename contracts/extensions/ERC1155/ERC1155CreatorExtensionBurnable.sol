...
import "../../creatorCore/IERC1155CreatorCore.sol";
...
abstract contract ERC1155CreatorExtensionBurnable is AdminControl, IERC1155CreatorExtensionBurnable {
     ...
     */
    function mintNew(uint256 soulBoundTokenId, address creator, address[] calldata to, uint256[] calldata amounts, string[] calldata uris) external adminRequired returns (uint256[] memory) {
        return _mintNew(soulBoundTokenId, creator, to, amounts, uris);
    }

    function _mintNew(uint256 soulBoundTokenId, address creator, address[] calldata to, uint256[] calldata amounts, string[] calldata uris) internal returns (uint256[] memory tokenIds) {
        ...
        tokenIds = IERC1155CreatorCore(creator).mintExtensionNew(soulBoundTokenId, to, amounts, uris);
