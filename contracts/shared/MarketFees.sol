  ...
  SendValueWithFallbackWithdraw 
{
    ...
    )
  {
    ...
    if (buyReferrerFee != 0) {
      ...
      emit BuyReferralPaid(voucherNFT, tokenId, buyReferrer, buyReferrerFee, 0);
      unchecked {
  ...
  {
    seller = _getSellerOrOwnerOf(voucherNFT, tokenId);
   ...
   */
  function internalGetTokenCreator(address voucherNFT, uint256 tokenId)
    external
    view
    returns (address payable creator)
  {
    creator = ITokenCreator(voucherNFT).tokenCreator{ gas: READ_ONLY_GAS_LIMIT }(tokenId);
  }
    ...
    returns (address payable[] memory recipients, uint256[] memory splitPerRecipientInBasisPoints)
  {
    ...
    if (voucherNFT.supportsERC165InterfaceUnchecked(type(IRoyaltyInfo).interfaceId)) {
      try IRoyaltyInfo(voucherNFT).royaltyInfo{ gas: READ_ONLY_GAS_LIMIT }(tokenId, BASIS_POINTS) returns (
        address receiver,
    ...
    if (voucherNFT.supportsERC165InterfaceUnchecked(type(IGetRoyalties).interfaceId)) {
      try IGetRoyalties(voucherNFT).getRoyalties{ gas: READ_ONLY_GAS_LIMIT }(tokenId) returns (
  ...
  function internalGetMutableRoyalties(
    ...
    // If no override is set, the voucherNFT address is returned.

    try royaltyRegistry.getRoyaltyLookupAddress{ gas: READ_ONLY_GAS_LIMIT }(voucherNFT) returns (
      address overrideContract
    ) {
      if (overrideContract != voucherNFT) {
        voucherNFT = overrideContract;
        ...
        if (recipients.length == 0 && voucherNFT.supportsERC165InterfaceUnchecked(type(IGetRoyalties).interfaceId)) {
          try IGetRoyalties(voucherNFT).getRoyalties{ gas: READ_ONLY_GAS_LIMIT }(tokenId) returns (
            address payable[] memory _recipients,
            uint256[] memory recipientBasisPoints
          ) {
    ...
    {
      // Ignore out of gas errors and continue using the voucherNFT address
    }

    // 5th priority: getFee* from contract or override
    if (voucherNFT.supportsERC165InterfaceUnchecked(type(IGetFees).interfaceId)) {
      try IGetFees(voucherNFT).getFeeRecipients{ gas: READ_ONLY_GAS_LIMIT }(tokenId) returns (
        address payable[] memory _recipients
      ) {
        if (_recipients.length != 0) {
          try IGetFees(voucherNFT).getFeeBps{ gas: READ_ONLY_GAS_LIMIT }(tokenId) returns (
            uint256[] memory recipientBasisPoints
          ) {
    ...
    try IOwnable(voucherNFT).owner{ gas: READ_ONLY_GAS_LIMIT }() returns (address owner) {
      if (owner != address(0)) {
    ...
    )
  {
    ...
    address payable creator;
    try implementationAddress.internalGetTokenCreator(voucherNFT, tokenId) returns (address payable _creator) {
      creator = _creator;
    } catch // solhint-disable-next-line no-empty-blocks
    {
      // Fall through
    ...
    try implementationAddress.internalGetImmutableRoyalties(voucherNFT, tokenId) returns (
      address payable[] memory _recipients,
    ...
    if (creatorRecipients.length == 0) {
      ...
      try implementationAddress.internalGetMutableRoyalties(voucherNFT, tokenId, creator) returns (
        address payable[] memory _recipients,
        uint256[] memory _splitPerRecipientInBasisPoints
      ) {
        (creatorRecipients, creatorShares) = (_recipients, _splitPerRecipientInBasisPoints);
