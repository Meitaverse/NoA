...
interface IFollowModule {
     ...
     *
     * @return bytes The encoded data to emit in the manager.
     */
    function initializeFollowModule(uint256 soulBoundTokenId, bytes calldata data)
        external
        returns (bytes memory);

    /**
     * @notice Processes a given follow, this can only be called from the Manager contract.
     *
     * @param follower The follower address.
     * @param soulBoundTokenId The token ID of the profile being followed.
     ...
     * only be called from the Manager contract.
     *
     ...
     *
     * @param soulBoundTokenId The token ID of the profile associated with the follow NFT being transferred.
     ...
     *          - Fetches the follow module from the manager.
     ...
     *
     * @param soulBoundTokenId The token ID of the profile to validate the follow for.
