...
export async function followReturningTokenIds({
  ...
  let tokenIds;
  if ('sig' in vars) {
    tokenIds = await manager.connect(sender).callStatic.followWithSig(vars);
    await expect(manager.connect(sender).followWithSig(vars)).to.not.be.reverted;
  } else {
    tokenIds = await manager.connect(sender).callStatic.follow(vars.profileIds, vars.datas);
    await expect(manager.connect(sender).follow(vars.profileIds, vars.datas)).to.not.be.reverted;
  }
  return tokenIds;
}
...
export async function commentReturningTokenId({
  ...
  let tokenId;
  if ('sig' in vars) {
    tokenId = await manager.connect(sender).callStatic.commentWithSig(vars);
    await expect(manager.connect(sender).commentWithSig(vars)).to.not.be.reverted;
  } else {
    tokenId = await manager.connect(sender).callStatic.comment(vars);
    await expect(manager.connect(sender).comment(vars)).to.not.be.reverted;
  }
  return tokenId;
}
...
export async function mirrorReturningTokenId({
  ...
  let tokenId;
  if ('sig' in vars) {
    tokenId = await manager.connect(sender).callStatic.mirrorWithSig(vars);
    await expect(manager.connect(sender).mirrorWithSig(vars)).to.not.be.reverted;
  } else {
    tokenId = await manager.connect(sender).callStatic.mirror(vars);
    await expect(manager.connect(sender).mirror(vars)).to.not.be.reverted;
  }
  return tokenId;
}
...
export async function postReturningTokenId({
  ...
  let tokenId;
  if ('sig' in vars) {
    tokenId = await manager.connect(sender).callStatic.postWithSig(vars);
    await expect(manager.connect(sender).postWithSig(vars)).to.not.be.reverted;
  } else {
    tokenId = await manager.connect(sender).callStatic.post(vars);
    await expect(manager.connect(sender).post(vars)).to.not.be.reverted;
  }
  return tokenId;
}
